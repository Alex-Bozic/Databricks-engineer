...
traffic_df = df.groupBy("traffic_source").agg(round(sum("revenue"), 1).alias("total_rev"), avg("revenue").alias("avg_rev"))
...
# TODO
final_df = (top_traffic_df.withColumn("total_rev", round((col("total_rev")*100).cast("long")/100, 2)).withColumn("avg_rev", round((col("avg_rev")*100).cast("long")/100, 2))
)
